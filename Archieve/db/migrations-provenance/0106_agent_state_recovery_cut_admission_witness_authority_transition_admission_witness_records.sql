-- 0106_agent_state_recovery_cut_aw_authority_transition_witness_records.sql
-- Witness-certified accountability records for recovery-cut admission witness authority-transition admission rows.

CREATE TABLE IF NOT EXISTS agent_state.recovery_cut_aw_authority_transition_witness_records (
...
CREATE INDEX IF NOT EXISTS recovery_cut_aw_authority_transition_witness_scope_idx
  ON agent_state.recovery_cut_aw_authority_transition_witness_records (
    tenant_id,
    transition_admission_witness_store_id,
...
CREATE INDEX IF NOT EXISTS recovery_cut_aw_authority_transition_witness_admission_idx
  ON agent_state.recovery_cut_aw_authority_transition_witness_records (
    tenant_id,
    transition_admission_store_id,
    admission_sequence,
...
CREATE INDEX IF NOT EXISTS recovery_cut_aw_authority_transition_witness_authority_idx
  ON agent_state.recovery_cut_aw_authority_transition_witness_records (
    tenant_id,
    recovery_cut_admission_witness_authority_topology_id,
    authority_sequence,
...
CREATE OR REPLACE FUNCTION agent_state.prevent_recovery_cut_aw_authority_transition_witness_record_rewrite()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS prevent_recovery_cut_aw_authority_transition_witness_record_rewrite
  ON agent_state.recovery_cut_aw_authority_transition_witness_records;

CREATE TRIGGER prevent_recovery_cut_aw_authority_transition_witness_record_rewrite
  BEFORE UPDATE OR DELETE ON agent_state.recovery_cut_aw_authority_transition_witness_records
  FOR EACH ROW
  EXECUTE FUNCTION agent_state.prevent_recovery_cut_aw_authority_transition_witness_record_rewrite();

REVOKE INSERT, UPDATE, DELETE ON agent_state.recovery_cut_aw_authority_transition_witness_records FROM PUBLIC;

COMMENT ON TABLE agent_state.recovery_cut_aw_authority_transition_witness_records IS
...
COMMENT ON COLUMN agent_state.recovery_cut_aw_authority_transition_witness_records.admission_record_hash IS
...
COMMENT ON COLUMN agent_state.recovery_cut_aw_authority_transition_witness_records.next_authority_topology_hash IS
  'Hash of the authority topology derived by the transition-admission record being witnessed.';

COMMENT ON COLUMN agent_state.recovery_cut_aw_authority_transition_witness_records.admission_certificate IS
  'Quorum certificate over the exact transition-admission store, topology id, admission sequence, and admission record hash.';

COMMENT ON COLUMN agent_state.recovery_cut_aw_authority_transition_witness_records.witness_record_hash IS
