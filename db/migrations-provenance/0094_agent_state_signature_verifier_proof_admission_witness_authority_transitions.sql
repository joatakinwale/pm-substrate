-- 0094_agent_state_sig_verifier_proof_aw_authority_transitions.sql
-- Durable authority transitions for signature-verifier proof admission witness topology.

CREATE TABLE IF NOT EXISTS agent_state.sig_verifier_proof_aw_authority_transitions (
...
CREATE INDEX IF NOT EXISTS signature_verifier_proof_admission_witness_authority_scope_idx
  ON agent_state.sig_verifier_proof_aw_authority_transitions (
    tenant_id,
    proof_admission_witness_authority_topology_id,
    authority_scope,
    authority_sequence
...
CREATE TRIGGER prevent_signature_verifier_proof_admission_witness_authority_rewrite
  BEFORE UPDATE OR DELETE ON agent_state.sig_verifier_proof_aw_authority_transitions
  FOR EACH ROW
  EXECUTE FUNCTION agent_state.prevent_signature_verifier_proof_admission_witness_authority_rewrite();

REVOKE INSERT, UPDATE, DELETE ON agent_state.sig_verifier_proof_aw_authority_transitions FROM PUBLIC;

COMMENT ON TABLE agent_state.sig_verifier_proof_aw_authority_transitions IS
...
COMMENT ON COLUMN agent_state.sig_verifier_proof_aw_authority_transitions.authority_record_hash IS
...
COMMENT ON COLUMN agent_state.sig_verifier_proof_aw_authority_transitions.effective_from_admission_sequence IS
  'First signature-verifier proof admission sequence for which this authority transition may authorize witness certificates.';

COMMENT ON COLUMN agent_state.sig_verifier_proof_aw_authority_transitions.transition IS
