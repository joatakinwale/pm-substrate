CREATE TABLE IF NOT EXISTS agent_state.pt_hsh_ptsh_ptsh_witness_observations (
...
CREATE INDEX IF NOT EXISTS pt_hsh_ptsh_ptsh_witness_head_idx
  ON agent_state.pt_hsh_ptsh_ptsh_witness_observations (
    tenant_id,
    pruning_tombstone_sequence,
    pruning_tombstone_record_hash
  );

CREATE INDEX IF NOT EXISTS pt_hsh_ptsh_ptsh_witness_observer_idx
  ON agent_state.pt_hsh_ptsh_ptsh_witness_observations (
    tenant_id,
    observer_id,
    witness_sequence
...
CREATE INDEX IF NOT EXISTS pt_hsh_ptsh_ptsh_witness_status_idx
  ON agent_state.pt_hsh_ptsh_ptsh_witness_observations (
    tenant_id,
    status,
    witness_sequence
...
COMMENT ON TABLE agent_state.pt_hsh_ptsh_ptsh_witness_observations IS
  'Hash-linked witness observations for replay-recovering the current v133 pruning tombstone-store head after agent amnesia.';

COMMENT ON COLUMN agent_state.pt_hsh_ptsh_ptsh_witness_observations.pruning_tombstone_store_head IS
  'Observed v133 pruning tombstone-store head. It is operational only when replay admits the witness record.';

COMMENT ON COLUMN agent_state.pt_hsh_ptsh_ptsh_witness_observations.consistency_proof IS
  'Replay proof from the beginning or latest witnessed head to the observed v133 pruning tombstone-store head.';

COMMENT ON COLUMN agent_state.pt_hsh_ptsh_ptsh_witness_observations.signature IS
...
COMMENT ON COLUMN agent_state.pt_hsh_ptsh_ptsh_witness_observations.decision IS
