...
|---|---|---|---|---|
| queued (external app anchors) | `D6` | Re-establish per-revision lab boundary conformance | [`ROADMAP.md`](./ROADMAP.md#up-next--d6d7-execution-sequence) | PluggedInSocial restores `browser_qa_harness` + `operatorRunMonitorSurface`; ArrowHedge restores the neutral `/integration/v1` contract; each `pm:boundary` run produces a green revision-pinned receipt |
