...
| --- | --- | --- | --- |
| Agent-state Arrowsmith | `research/daily-arrowsmith-agent-state/index.md` | Active. v10 audited the replay-backed verification catalog and write-transport coverage implementation now on `main`, marked the v09 code slice partially closed, and added bridges from certificate-bound admission, cross-channel delivery failure, memory-control-flow attacks, evidence-first diagnosis, state-based real-environment benchmarking, AgentOps, MAS marginal-utility evaluation, and human-AI teamwork/scaffolding field experiments. | Promote fixture-backed verification into durable stores or certificate-like admission artifacts, add target-side delivery confirmation, memory-control-flow review, state-based final-environment checks, role-utility metrics, and PM protocol-burden/rework measurements. |
