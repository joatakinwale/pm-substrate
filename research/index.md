...
| --- | --- | --- | --- |
| Agent-state Arrowsmith | `research/daily-arrowsmith-agent-state/index.md` | Active. v12 audited v11's certificate-bound replay proof against June 2026 memory-poisoning, memory-control-flow, workflow-verification, multimodal-memory, credential/status, MCP, OpenTelemetry, and human-AI teaming sources. v13 closed the memory-write/read taxonomy seam, and v14 now partly closes the target-receipt seam with typed receipt evidence and dispatch-only downgrade warnings. | Promote fixture-backed certificates and receipts into durable live status stores, add policy-transition mini-specs, and measure final-state consistency plus PM protocol burden. |
...
|
`-- T6 final-state verification + PM burden
...
4. Promote the new committed-corpus-backed certificate/catalog verification into durable DB-backed or substrate-store-backed verification sources for state-review artifact ids/hashes, evidence-admission review ids, tenant/workflow binding, rejected-evidence policy disposition, policy version, revocation epoch, execution identity, and validity windows.
...
18. Watch whether agent infrastructure admits certificate/revocation artifacts before production mutation, not only audit logs after mutation.
